...
def nfsmount(ip):
	...
	if '*' in anonNFS or 'anonymous' in anonNFS:
		...
		subprocess.check_output('mount -t nfs '+ ip + ':' + results + ' mnt/', shell=True)
		print('[+] NFS share mounted to ./mnt')
		subprocess.check_output('cd mnt/', shell=True)
		fileSearch(results)
		# unmount share
		print('[+] Unmounting mount: ' + results)
		subprocess.check_output('umount mnt/', shell=True)
	else:
		print('[-] No Anonymous NFS shares found')


def fileSearch(results):
	root = 'mnt/'
	print('[+] Searching for files in mount...')
	for path, subdirs, files in os.walk(root):
		for name in files:
			ssnSearch(results, name)
			CCSearch(results, name)
...
def CCSearch(results, name):
	print('[+] Looking for Credit Card Numbers in: ' + name)
	with open('mnt/' + name, 'r') as f:
...
def ssnSearch(results, name):
	print('[+] Looking for SSNs in: ' + name)
	with open('mnt/' + name, 'r') as f:
		for line, number in enumerate(f, 1):
